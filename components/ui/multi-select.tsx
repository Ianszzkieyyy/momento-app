...
export function MultiSelectContent({
  ...
  const { searchValue, setSearchValue, toggleValue, items, onItemAdded } = useMultiSelectContext()

    // Check if search value exists in current items
  const searchValueExists = searchValue && [...items.keys()].some(
    ...
    <>
      ...
      <PopoverContent className="min-w-[var(--radix-popover-trigger-width)] p-0">
        <Command {...props}>
          ...
          <CommandList>
            {children}
            {canSearch && allowCreate && searchValue && !searchValueExists && (
              <>
                <CommandSeparator />
                <CommandItem
